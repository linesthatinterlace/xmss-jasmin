...
- **Write specs from the source, not the implementation.** SPEC.md was derived from RFC 8391 and design decisions, deliberately without reading the Jasmin code. This ensures the spec is an independent check — "can't mark our own homework." The same principle applies to test oracles: derive expected values from the RFC or reference implementation, not from the code under test.
- **Verify CI end-to-end before declaring a task complete.** Pushing and watching the run caught real deployment concerns (job dependency structure, parallel build opportunities) that wouldn't surface from local testing alone. "Works locally" is not "done."
...
- **Discover all project documents at session start.** Missing key docs meant missing the full implementation order, phase gates, and cross-implementation verification requirements. At the start of a session, glob for `*.md` in the working directory — don't assume CLAUDE.md is the only guide.
