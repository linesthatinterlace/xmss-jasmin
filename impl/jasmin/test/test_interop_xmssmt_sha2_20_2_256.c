/**
 * test_interop_xmssmt_sha2_20_2_256.c — Cross-implementation verification (C ↔ Jasmin)
 *
 * Links against both libxmss.a (C impl) and xmssmt_sha2_20_2_256.s (Jasmin).
 * Tests:
...
#define C_SK_BYTES  135   /* XMSS-MT: 4 + 3 (idx_bytes) + 4*32 = 135 */
#define C_SIG_BYTES 4963

/* Jasmin-side sizes */
#define J_PK_BYTES      JADE_SIGN_XMSSMT_SHA2_20_2_256_PUBLICKEYBYTES
#define J_SK_BYTES      JADE_SIGN_XMSSMT_SHA2_20_2_256_SECRETKEYBYTES
#define J_SIG_BYTES     JADE_SIGN_XMSSMT_SHA2_20_2_256_BYTES
#define J_STATE_BYTES   JADE_SIGN_XMSSMT_SHA2_20_2_256_STATEBYTES
#define J_SCRATCH_BYTES JADE_SIGN_XMSSMT_SHA2_20_2_256_SCRATCHBYTES
...
int main(void)
{
    ...
    /* C-side buffers — xmss_mt_state is ~780KB, heap-allocate */
    xmss_params p;
    xmss_mt_params_from_oid(&p, OID_XMSS_MT_SHA2_20_2_256);
    uint8_t pk_c[C_PK_BYTES], sk_c[C_SK_BYTES];
    uint8_t sig_c[C_SIG_BYTES];
    xmss_mt_state *state_c = (xmss_mt_state *)calloc(1, sizeof(xmss_mt_state));
    if (!state_c) {
        printf("FAIL: could not allocate C MT state\n");
        return 1;
    }
    ...
    printf("=== XMSSMT-SHA2_20/2_256 Cross-Implementation Interop ===\n\n");
    ...
    /* C keygen */
    memset(state_c, 0, sizeof(xmss_mt_state));
    ret = xmss_mt_keygen(&p, pk_c, sk_c, state_c, BDS_K, replay_randombytes);
    if (ret != XMSS_OK) {
    ...
    jret = jade_sign_xmssmt_sha2_20_2_256_amd64_ref_keypair(
    ...
    /* Full PK comparison (OIDs now match) */
    ...
    memset(state_c, 0, sizeof(xmss_mt_state));
    xmss_mt_keygen(&p, pk_c, sk_c, state_c, BDS_K, replay_randombytes);

    memset(state_j, 0, J_STATE_BYTES);
    memset(scratch, 0, J_SCRATCH_BYTES);
    jade_sign_xmssmt_sha2_20_2_256_amd64_ref_keypair(
        pk_j, sk_j, state_j, seeds, scratch);

    ret = xmss_mt_sign(&p, sig_c, msg, sizeof(msg), sk_c, state_c, BDS_K);
    if (ret != XMSS_OK) {
        printf("FAIL (C sign returned %d)\n", ret);
        pass = 0;
    } else {
        memset(scratch, 0, J_SCRATCH_BYTES);
        jret = jade_sign_xmssmt_sha2_20_2_256_amd64_ref_open(
            msg, sizeof(msg), sig_c, pk_j, scratch);
        if (jret == 0) {
            printf("OK\n");
        } else {
    ...
    memset(scratch, 0, J_SCRATCH_BYTES);
    jret = jade_sign_xmssmt_sha2_20_2_256_amd64_ref(
        sig_j, msg, sizeof(msg), sk_j, state_j, scratch);
    if (jret != 0) {
        printf("FAIL (Jasmin sign returned %lu)\n", (unsigned long)jret);
        pass = 0;
    } else {
        ret = xmss_mt_verify(&p, msg, sizeof(msg), sig_j, pk_j);
        if (ret == XMSS_OK) {
            printf("OK\n");
        } else {
    ...
    fflush(stdout);
    {
        ...
        memset(scratch, 0, J_SCRATCH_BYTES);
        jret = jade_sign_xmssmt_sha2_20_2_256_amd64_ref_open(
            bad_msg, sizeof(bad_msg), sig_c, pk_j, scratch);
        if (jret != 0) {
            printf("OK\n");
        } else {
    ...
    fflush(stdout);
    {
        ...
        memcpy(bad_msg, msg, sizeof(msg));
        bad_msg[0] ^= 0x01;
        ret = xmss_mt_verify(&p, bad_msg, sizeof(bad_msg), sig_j, pk_j);
        if (ret != XMSS_OK) {
            printf("OK\n");
        } else {
